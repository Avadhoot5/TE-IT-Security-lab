...
phi(n) or t is the totient. totient 't' is a number smaller than 'n', such that 't' shares no common factors with 'n' (co-primes). For a prime number 'p', t = p-1 (Eulers totient function) hence phi(n) = (p-1).(q-1), since n = p.q
...
if ciphertext == None:
    ...
    print("private exponential: {}\npublic exponential: {}\nmodulus: {}\nciphertext: {}".format(rsa.e, rsa.d, rsa.n, enctext))
elif plaintext == None:
    dectext = rsa.decrypt(ciphertext)
    print("private exponential: {}\npublic exponential: {}\nmodulus: {}\nplaintext: {}".format(rsa.e, rsa.d, rsa.n, dectext))
